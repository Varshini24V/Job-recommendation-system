...
from bedrock_utils import (titan_embedding,summarize_resume)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        response = s3.get_object(Bucket=S3_BUCKET,Key=resume_id)
        ...
        embedding = titan_embedding(text[:8000])

        if not embedding:
            return error_response(500, "Embedding generation failed")
        print("Embedding generated")
        print("Embedding size:",len(embedding))

        # GENERATE SUMMARY
        print("Generating AI summary...")
        summary = summarize_resume(text[:4000])
        if not summary:
