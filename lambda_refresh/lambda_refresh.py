...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not APP_ID or not APP_KEY:
            return {
                "statusCode": 500,
                "body": json.dumps({"error":"Missing Adzuna credentials"})
            }
        # Fetch Jobs
        ...
        for job in jobs_data:
            try:
                ...
                embedding = titan_embedding(description[:8000])
                document = {
                ...
                print("Job Insert Error:", str(job_error))

        print(f"Inserted/Updated {inserted} jobs")
